...
### ~~P3 — Remove now-dead waitlist UI~~ — DONE (funnel-switch PR, 2026-07-23)
- Shipped with the intake funnel switch: waitlist branches removed from Taxi/Report/Admin, `isWaitlisted` + the 'waitlist' gate reason deleted (gateReason is now granted|needs_intake). The DB `waitlist` status value + add_cohort_cap_trigger.sql remain, per the re-enable note.
