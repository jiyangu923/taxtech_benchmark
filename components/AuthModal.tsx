...
const AuthModal: React.FC<AuthModalProps> = ({ isOpen, onClose, onLogin }) => {
  ...
  const handleGoogleSignIn = () => {
    ...
    if (!clientId || !googleLib) {
      // No real OAuth configured — use mock Google sign-in flow
      setError(null);
  ...
  const handleMockGoogleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await new Promise(resolve => setTimeout(resolve, 600));
      const user = mockStore.loginWithGoogle(mockGoogleEmail);
      setIsSuccess(true);
      await new Promise(resolve => setTimeout(resolve, 600));
          ...
          {isSuccess ? (
            ...
            <div className="space-y-6">
              <div className="flex items-center gap-3 p-4 bg-blue-50 border border-blue-100 rounded-xl">
                ...
                <p className="text-xs font-bold text-blue-700">Enter your Google email to continue</p>
              </div>
              {error && (
              ...
              <form onSubmit={handleMockGoogleSubmit} className="space-y-4">
                <div className="space-y-1.5">
                  <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest ml-1">Google Email</label>
                  <div className="relative group">
                    ...
                    <input
                      type="email" required autoFocus placeholder="you@gmail.com"
                      ...
                      value={mockGoogleEmail} onChange={(e) => setMockGoogleEmail(e.target.value)}
                    />
                  </div>
                </div>
                <button
                  type="submit" disabled={isLoading}
                  className="w-full flex items-center justify-center gap-2 py-4 bg-primary text-white rounded-2xl font-black text-sm uppercase tracking-widest hover:bg-indigo-900 transition-all shadow-xl shadow-primary/20 active:scale-[0.98] disabled:opacity-70"
                >
                  {isLoading ? <Loader2 className="h-5 w-5 animate-spin" /> : 'Continue with Google'}
                </button>
              </form>
              <div className="text-center">
                <button onClick={() => setShowMockGoogle(false)} className="text-sm font-bold text-gray-400 hover:text-gray-600">
                  Back to sign in
                </button>
              </div>
            </div>
