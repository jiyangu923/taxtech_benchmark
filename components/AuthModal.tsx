...
import { X, Mail, Lock, ShieldCheck, ChevronRight, AlertCircle, Eye, EyeOff, Loader2, CheckCircle2, UserPlus, LogIn } from 'lucide-react';
  ...
  const handleAuthAction = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (mode === 'signup') {
        ...
        mockStore.loginWithGoogle(email);
      }
          ...
          <div className="flex justify-between items-center mb-8">
            <div>
              <h2 className="text-2xl font-black text-gray-900 tracking-tight">
                {mode === 'login' ? 'Welcome Back' : mode === 'signup' ? 'Create Account' : 'Identity Verification'}
              </h2>
              <p className="text-sm text-gray-500 mt-1 font-medium">
                {mode === 'login' ? 'Sign in to your benchmarking portal' : mode === 'signup' ? 'Join the industry benchmark study' : 'Sign in with your organizational Google account'}
              </p>
            </div>
            {!isLoading && (
          ...
          {isSuccess ? (
            ...
            <div className="space-y-6">
              ...
              <form onSubmit={handleAuthAction} className="space-y-4">
                ...
                >
                  {isLoading ? <Loader2 className="h-5 w-5 animate-spin" /> : mode === 'login' ? 'Sign In' : mode === 'signup' ? 'Create Account' : 'Verify & Sign In'}
                </button>
              </form>
              ...
              <div className="text-center pt-2">
                {mode === 'login' ? (
                  ...
                  <button onClick={() => setMode('login')} className="text-sm font-bold text-gray-400 hover:text-gray-600 flex items-center justify-center gap-2 w-full">
                    <LogIn className="h-4 w-4" /> Return to standard login
                  </button>
                )}
              </div>
            </div>
